...
function buildSystemPrompt({ question, context, templates }) {
  if (context) {
    return templates.ragSystemTemplateWithContext.replace("{context}", context);
  }
  return templates.ragSystemTemplateNoContext.replace("{question}", question);
}
